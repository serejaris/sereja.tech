...
| 2026-07-24 | M17 task-first blog navigation and social footer | `layouts/blog/list.html`, `static/search.js`, `layouts/_default/baseof.html`, `docs/PLAN.md`, `docs/STATUS.md` | `node --check static/search.js`; `hugo build`; canonical, sitemap, target-link, and redirect-source checks; rendered `/blog/` scan | pass locally; production smoke follows Git-connected `main` push | verify clusters and footer on `https://sereja.tech/blog/` |
