...
| 2026-08-04 | M18 Reading Ledger article redesign and production rollout | `layouts/blog/single.html`, `layouts/_default/baseof.html`, `layouts/_default/_markup/render-heading.html`, `static/article.js`, `static/images/blog/best-practices-code-ledger.webp`, `content/blog/best-practices-code.md`, `hugo.toml`, `design-qa.md`, `docs/PLAN.md`, `docs/STATUS.md`; PR `#155`; merge `6794bac` | `hugo build`; `node --check static/article.js`; rendered-HTML audit; local browser desktop/mobile interaction and route smoke; full Python SEO helper stack; `git diff --check`; Vercel preview checks; Git-connected production deployments; cache-busted production HTML, asset, redirect, sitemap, and taxonomy smoke | pass: design QA passed; production returns the Reading Ledger HTML with five H2 permalinks, desktop/mobile TOCs, three related rows, canonical slash, `308` slash redirect, one sitemap entry, and `noindex, follow` taxonomy policy | monitor production behavior and search signals |
...
| 2026-09-05 | M21 Defense Factory standalone guide rendering | `content/blog/ai-agent-defense-factory-guide.html`, `docs/blog/ai-agent-defense-factory-guide/preview-*.png`, `docs/PLAN.md`, `docs/STATUS.md` | `hugo --gc --minify`; Playwright direct-file and Hugo review at 1440×900; prompt copy interaction; `git diff --check` | pass: direct `file://` view hides frontmatter, uses the 740 px paper layout, and preserves the A4 prompt sidecar; mobile review skipped by founder request | open visual-review PR |
| 2026-09-05 | M21 Defense Factory reader-first copy | `content/blog/ai-agent-defense-factory-guide.html`, companion source notes and desktop previews; PR `#176` | editorial audit; Playwright direct-file and Hugo review at 1440×900; title and TOC consistency; source-link placement; Hugo build; `git diff --check` | pass: opening addresses an app security check before launch, the two-account example explains the workflow, lecture timestamps are absent, six source links are in the final section; mobile review skipped by founder request | update PR `#176` for author review |
