...
| Харнесс | Размер index.html | Время |
|---|---|---|
| Codex CLI 0.149.0 | 14 610 B | ~5.5 мин |
| Hermes agent v0.20.5 | 19 191 B | ~9.4 мин |
| Claude Code 2.1.239 | 76 823 B | ~57 мин |
| OMP 17.4.2 | 652 681 B | ~80 мин |
| Cline CLI 3.0.56, OpenCode 1.18.21 | ещё дописывали на момент публикации | пока нет |

Выдумывать остальным двум размеры и время не стану. Обновлю таблицу, когда прогоны финишируют.

Каждый артефакт — живая страница. Вот что получилось у четырёх финишировавших:
...
Повторить это можно одним промптом агенту. Порядок я дал бы такой. Один чистый шаблон контейнера вместо шести ручных установок: сделал раз, дальше только клоны. Сверка IP-плана со всеми существующими конфигами до запуска. Один ключ, экспорт через profile.d, DNAT-правила отдельным юнитом, алиасы на рабочей машине. И только потом нонс-тест каждого харнесса короткой командой: `omp -p "..."`, `claude -p "..."`, `codex exec "..."`, `cline --json --timeout N "..."`, `hermes -z "..."`, `opencode run "..."`.
