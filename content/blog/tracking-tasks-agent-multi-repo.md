...
Сегодня я запустил такой трек — партнёрская образовательная программа на апрель-май. Три репо: деки, программа, CRM. Сам change set pattern я не выдумал и не нашёл ручным гуглением. Попросил агента сделать research:
...
Агент нашёл официальный change set pattern от GitHub (Well-Architected), синтезировал из него пять элементов, записал в `~/.claude/rules/multi-repo-initiatives.md`. Дальше этот файл автоматически попадает в контекст любой новой сессии — правила из `~/.claude/rules/` агент читает сам, без специальной просьбы. Когда пришло время применить — агент уже знал форму и предложил. Я согласился. Он создал Project, выбрал репо для эпика (программа → репо программы), завёл label в трёх репо, расставил блоки Tracking.

Итог: три ссылки (доска, эпик, label) и работающий трек. Каждый коммит с префиксом, каждая новая задача подцепляется к эпику.

## Через месяц

Когда через месяц понадобится вспомнить контекст — агент уже знает, куда смотреть: Project для текущего статуса, эпик для overview, label для истории issues, коммиты с префиксом для хронологии. Собирает статус и отдаёт. Без раскопок. Форма сама подсказывает агенту, где контекст.
