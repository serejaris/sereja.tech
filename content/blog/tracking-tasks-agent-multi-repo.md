...
## Это же работает и для чистого кода

У меня пример с образовательной программой, потому что именно его я сегодня запустил — но форма не зависит от домена. Любая фича, которую тянешь через фронтенд, бэкенд и общий типизированный пакет, — уже multi-repo инициатива. Пять элементов работают там так же: один Project собирает задачи из всех трёх репо, эпик живёт в том, где основной changelog фичи (обычно бэкенд или фронтенд, зависит от того, кто её владельцем становится), label одинаковый везде, префикс в коммитах даёт историю по фиче через все три репо, блоки Tracking в README-ах.

Представь: бэкенд пишет новый эндпоинт, фронтенд меняет UI под него, shared-types добавляет типы — раньше это разваливалось в три отдельных потока, которые нужно руками синхронизировать. С этой формой — три sub-issue под одним эпиком, один Project board с колонками «backend / frontend / types», и ты (или любой член команды) видит всю фичу в одном месте. То же самое для инфра-миграции через API + web-app + docs, или B2B-продукта через лендинг + админку + биллинг.

Форма одна, домены разные.
...
Технически сама возможность сцеплять задачи из разных репо появилась у GitHub в [апреле 2025](https://github.blog/changelog/2025-04-09-evolving-github-issues-and-projects/), когда sub-issues вышли в общий доступ, а в [сентябре](https://github.blog/changelog/2025-09-11-a-rest-api-for-github-projects-sub-issues-improvements-and-more/) появился REST API и улучшения для работы между репо. Без этого дерево подзадач из разных мест было невозможно. Но это просто technical capability — инструмент. Скрепляющая деталь — правила в файле.
