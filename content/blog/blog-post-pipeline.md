...
knowledge:
  problem: "Регулярная публикация в блог занимает весь вечер и приводит к выгоранию"
  solution: "Sequential pipeline из 5 фаз с субагентами: вопросы, research, черновик, деаификация, публикация"
  pattern: "sequential-content-pipeline"
  tools: ["Claude Code", "Exa AI", "Telegram", "launchd"]
  takeaways:
    - "55 статей прошли через один и тот же 5-фазный пайплайн"
    - "4 параллельных критика в фазе деаификации находят разные проблемы в тексте"
    - "Субагент на Haiku генерирует Telegram-превью — быстрая и дешёвая модель для простых задач"
    - "Skills хранятся в ~/.claude/skills/ как markdown-инструкции для повторяемости"
    - "Посты публикуются автоматически в 19:00 МСК по одному из очереди"
  metrics:
    articles_count: 55
    pipeline_phases: 5
    deaify_critics: 4
  related:
    - slug: "multi-agent-text-deaify"
      relation: "детальное описание фазы деаификации с параллельными критиками"
    - slug: "ai-writing-pipelines"
      relation: "исследование 12 паттернов конвейеров AI-контента"
---
