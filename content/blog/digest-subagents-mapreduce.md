...
knowledge:
  problem: "Одиночный запрос к LLM для дайджеста 954 сообщений теряет контекст и галлюцинирует детали"
  solution: "MapReduce через 7 параллельных Haiku-субагентов с reply_to контекстом и финальной верификацией фактов"
  pattern: "mapreduce-digest-pipeline"
  tools: ["Claude Code", "Claude Haiku", "Claude Sonnet", "Telegram API"]
  takeaways:
    - "954 сообщения сворачиваются в 4000 символов за ~$0.03"
    - "7 Haiku-субагентов параллельно обрабатывают кластеры по ~15 сообщений"
    - "Reply_to контекст критичен — без него URL без описания превращаются в галлюцинации"
    - "Haiku в 6 раз дешевле Sonnet ($0.25 vs $1.50 за 1M input токенов)"
    - "Верификация поймала 1 галлюцинацию из 13 проверенных фактов"
  metrics:
    messages_processed: 954
    cost_per_digest: 0.03
    haiku_agents: 7
    facts_verified: 13
    hallucinations_caught: 1
  related:
    - slug: "map-reduce-youtube-metadata"
      relation: "тот же MapReduce паттерн для обработки YouTube-транскриптов"
    - slug: "ai-writing-pipelines"
      relation: "MapReduce — один из 12 паттернов AI-пайплайнов"
    - slug: "content-plan-subagents-exa"
      relation: "субагенты для контент-планирования — тот же принцип делегирования"
---
