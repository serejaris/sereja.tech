---
title: "UX видео-уроков: что реально работает"
date: 2026-01-13
description: "Исследовал best practices для страниц видео-уроков. Что говорит NN/g про таймкоды, почему табы лучше скролла, и как не потерять студента на первых секундах."
tags: ["ux", "e-learning"]
section: UX
knowledge:
  problem: "Страница видео-урока без UX-исследования получается неструктурированной и неудобной"
  solution: "Спека на основе NN/g исследований: видео сверху, таймкоды рядом, контент по табам, сохранение прогресса"
  pattern: "research-first-ux-spec"
  tools: ["Claude Code", "NN/g research"]
  takeaways:
    - "Right-rail blindness: пользователи игнорируют правую колонку — видео ставить слева/сверху"
    - "Длительность видео показывать до клика, рядом с превью"
    - "Таймкоды обязательны для записей длиннее 10 минут"
    - "Табы лучше длинного скролла для видео + конспект + задания"
    - "Подход 'сначала рисёрч, потом агент' даёт лучший результат чем 'возьми шаблон'"
  related:
    - slug: "ai-student-progress-tracking"
      relation: "отслеживание прогресса студентов в образовательных продуктах"
---

Нужна была страница для воркшопа.

Сказал Claude Code: «возьми шаблон из mentor, там похожее».

Результат — каша. Проще было с нуля.

## Что нужно было

Страница для записи воркшопа: YouTube-видео, таймкоды, конспект, задания, ссылки. Я не смотрю код — смотрю работает или нет. Не работало.

Решил сначала почитать исследования. Сформулировать спеку, потом отдать агенту. И хорошо что так сделал.

## NN/g про видео

У Nielsen Norman Group есть статья [«Videos as Instructional Content»](https://www.nngroup.com/articles/instructional-video-guidelines/). Оттуда:

**Видео справа — мёртвая зона.** Называется «right-rail blindness». Пользователи научились игнорировать правую колонку — там обычно реклама. Один участник исследования нашёл видео только когда модератор показал пальцем. И разозлился: «Спрятали как будто».

**Показывай длительность до клика.** Не внутри плеера, а рядом с превью. Пять секунд на решение «смотреть или нет». Не знаешь что видео короткое — пропустишь.

**Без таймкодов никак.** Для записей длиннее 10 минут это база. Кликнул — прыгнул.

{{< callout insight >}}
Я сам ставил видео в правую колонку три года. Теперь понимаю почему engagement был низким.
{{< /callout >}}

## Про табы

Если на странице сразу видео, конспект, задания и ссылки — глаз не знает куда смотреть. Табы решают: показываешь одно, остальное прячешь.

И прогресс должен быть виден. Человек хочет знать сколько осталось.

## Что получилось

Написал спеку, отдал Claude Code с Opus 4.5. Четыре таба:

- **Запись:** видео слева, таймкоды справа. Кликаешь по таймкоду — прыгает. Активный подсвечивается.
- **Конспект** для тех кто не хочет пересматривать.
- **Задания** с чекбоксами, всё сохраняется.
- И **материалы** — ссылки в карточках.

{{< callout warning "Сначала рисёрч" >}}
Когда сказал агенту «возьми оттуда» — получил кашу. Когда попросил сначала изучить best practices — нашёл NN/g, и всё заработало.
{{< /callout >}}

## Коротко

1. Видео наверху и крупно — это главное. Таймкоды рядом.
2. Контент по табам, не километровый скролл.
3. Всё сохраняется между сессиями.
4. И да: сначала спека, потом агенты.

## Источники

- [NN/g: Videos as Instructional Content](https://www.nngroup.com/articles/instructional-video-guidelines/)
- [NN/g: Fight Right-Rail Blindness](https://www.nngroup.com/articles/fight-right-rail-blindness/)
- [Viartisan: eLearning UI/UX Design Guide 2025](https://viartisan.com/2025/05/27/elearning-ui-ux-design/)
