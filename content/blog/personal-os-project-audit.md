...
date: 2026-02-11
description: "Попросил агентов проанализировать 10 проектов — оказалось, всё уже было. 8 повторяющихся ядер, которые я не замечал."
tags: ["claude-code", "вайбкодинг", "паттерны", "personal-os"]
image: "/images/blog/personal-os-project-audit-preview.png"
---

Чтобы найти скрытые паттерны в своих проектах, нужно посмотреть на них все разом — не по одному. Я попросил двух агентов параллельно проанализировать ~10 активных проектов. За вечер они нашли 8 повторяющихся ядер — логику, которую я переписывал заново в каждом проекте, не замечая этого.

## Невидимый налог

У меня бот для рассылок, генератор дайджестов чата, платформа для управления учениками, конвертер видео в текст, поисковик по переписке — и ещё пяток инструментов помельче. Все работают. Каждый решает свою задачу.

Но внутри — одна и та же логика: разбить текст на куски перед отправкой в LLM, выбрать модель подешевле для подготовки и подороже для генерации, определить какое сообщение важнее другого.
...
## Ядро 3: Выбор модели под задачу
...
Пять ядер — одна цепочка. Измени что-то наверху — и это проливается на все проекты внизу.
...
Но само знание — уже рычаг. Теперь при создании нового проекта я первым делом сверяюсь с картой ядер: эта задача уже решена четырьмя способами, незачем изобретать пятый.
...
По данным RDW Software Engineering, после извлечения переиспользуемых ядер в реальном проекте дублирование снизилось на 38%, а переиспользование выросло на 33%. Компания Cyrus показала, что 6 AI-агентов за 3.5 часа справились с аудитом, на который у senior-инженера ушло бы 2-3 недели.

## Частые вопросы

### Нужно ли сразу рефакторить?

Нет. Аудит — это карта, а не план действий. Зафиксируй находки, используй при следующем проекте. Рефакторинг оправдан, когда одно и то же ядро мешает жить в 3+ проектах.
