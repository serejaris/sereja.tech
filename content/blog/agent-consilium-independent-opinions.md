...
knowledge:
  problem: "Один агент даёт одну перспективу на проблему без альтернативных точек зрения"
  solution: "Паттерн консилиума — несколько независимых субагентов-экспертов оценивают проект параллельно, затем синтез"
  pattern: "independent-expert-consilium"
  tools: ["Claude Code", "Exa MCP", "GitHub Issues"]
  takeaways:
    - "5 независимых экспертов находят разные проблемы, которые один агент пропустил бы"
    - "Сессия консилиума заняла 92 минуты и 15 промптов"
    - "Research после мнений превращает галлюцинации в проверенные факты"
    - "Результат — конкретные задачи в GitHub Issues, а не абстрактные рекомендации"
    - "Консилиум нужен когда ошибка стоит дней работы, для простых задач хватит одного агента"
  metrics:
    session_minutes: 92
    prompts_count: 15
    experts_count: 5
  prerequisites: ["Claude Code с субагентами", "Exa MCP для research"]
  related:
    - slug: "agent-teams-opus-4-6"
      relation: "Agent Teams — про скорость параллельной работы, консилиум — про качество решений"
    - slug: "multi-agent-text-deaify"
      relation: "параллельные критики в тексте — похожий паттерн для другой задачи"
    - slug: "llm-evals-for-vibecoder"
      relation: "четыре параллельных критика в eval-пайплайне"
---
