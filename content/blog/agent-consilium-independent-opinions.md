...
Я попробовал иначе. Показал текущее состояние проекта онлайн-школы нескольким субагентам и попросил каждого дать независимую оценку. Не выполнить задачу — а высказаться. Как врачи на консилиуме: каждый смотрит на одного пациента, но видит своё.
...
Проект онлайн-школы. Есть бот, уроки, платежи, прогресс учеников. Работает, но хаотично. Я хотел понять: что хорошо, что плохо, и куда двигаться дальше.

Первый промпт — попросил агента нарисовать текущую архитектуру.

{{< callout type="insight" >}}
Нарисуй, пожалуйста, как это будет работать. В виде аски диаграммы.
{{< /callout >}}

Получил картину. Увидел масштаб. И понял, что одного мнения мне мало.

{{< callout type="insight" >}}
Вот так вот выглядит сейчас. Что хорошо, что плохо? Нужен консилиум независимых экспертов через субагентов и после план составить действий.
{{< /callout >}}
...
{{< callout type="insight" >}}
А можешь по каждой из вот этих предлагаемых задач провести research через App Agent, чтобы понять, насколько, как бы, оно вообще необходимо и нужно.
{{< /callout >}}
...
{{< callout type="insight" >}}
Давай через brainstorm сделаем для каждой из этих задач план и запишем его в исю, чтобы потом подхватить.
{{< /callout >}}
