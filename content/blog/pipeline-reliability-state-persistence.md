...
knowledge:
  problem: "AI-пайплайны на скиллах падают на середине и теряют весь прогресс из-за переполнения контекста."
  solution: "State file в JSON сохраняет состояние после каждого шага, флаг --resume продолжает с места падения."
  pattern: "durable-execution-state-persistence"
  tools: ["Claude Code", "Opus 4.5", "Python", "ffmpeg", "Playwright", "ai-whisper"]
  takeaways:
    - "Пайплайн упал на 47-й минуте — 20 минут работы над субтитрами потеряны без state persistence"
    - "Каждый LLM-вызов стоит денег, контекст ограничен 200K токенов — потеря контекста = потеря бюджета"
    - "Параллельные шаги через ThreadPoolExecutor для независимых операций (thumbnail + slides)"
    - "Паттерн durable execution используется в Temporal, Restate, LangGraph"
  metrics:
    pipeline_steps: 5
    failure_minute: 47
  ...
  related:
    - slug: "openclaw-hooks-automation"
      relation: "Тот же принцип сохранения состояния, но для агента целиком через хуки"
---
