...
tags: ["промпты", "методология"]
knowledge:
  problem: "Итерация готового артефакта ломает оформление и тратит контекст агента впустую."
  solution: "Сначала создай текстовый черновик, итерируй структуру, затем генерируй финальный артефакт один раз."
  pattern: "draft-before-artifact"
  tools: ["Claude Code", "Opus 4.6"]
  takeaways:
    - "Двухстадийный подход (outline → генерация) даёт 95% успешности vs 74.6% без планирования"
    - "2-5 итераций черновика достаточно для большинства артефактов"
    - "Итерация в тексте занимает секунды, итерация готового артефакта — минуты"
    - "Паттерн универсален: презентации, код, изображения, блог-посты"
    - "Три промпта превращают сырую идею в готовый план для агента"
  related:
    - slug: "fix-once-rule-forever"
      relation: "черновик можно превратить в правило для агента"
---
