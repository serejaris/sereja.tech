...
section: LLM
knowledge:
  problem: "Текстовые инструкции в промпте не гарантируют выполнение — модель забывает хештег в 10-20% случаев"
  solution: "Использовать structured output с Pydantic/JSON Schema — модель физически не может вернуть невалидный ответ"
  pattern: "structured-output-schema"
  tools: ["Pydantic", "Gemini API", "JSON Schema"]
  takeaways:
    - "Текстовые промпты — намёки, а не гарантии: 1-2 сбоя из 10 запусков"
    - "Промпт сократился с 47 строк до 14 — формат ушёл в код, промпт только про контент"
    - "Всё что можно сделать кодом — делай кодом: модель генерирует контент, код гарантирует структуру"
    - "10% сбоев = 3 косяка в месяц при ежедневной публикации"
  metrics:
    prompt_lines_before: 47
    prompt_lines_after: 14
    failure_rate_text_percent: 10
