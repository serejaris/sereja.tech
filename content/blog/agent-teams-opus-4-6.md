...
knowledge:
  problem: "Субагенты общаются только через лидера и не могут координироваться напрямую"
  solution: "Agent Teams в Opus 4.6 — полноценная команда агентов с горизонтальной коммуникацией и shared task list"
  pattern: "horizontal-agent-teams"
  tools: ["Claude Code", "Opus 4.6", "Socket.IO", "React", "Vite", "Railway"]
  takeaways:
    - "Agent Teams включаются одной строкой в settings.json через переменную окружения"
    - "Чат на стриме собран за 15 минут двумя агентами — backend и frontend параллельно"
    - "Главная проблема параллельной разработки — event mismatch: контракт нужно фиксировать до старта"
    - "5-6 задач на teammate — золотая середина по документации Anthropic"
    - "Один файл — один агент, иначе конфликты и перезаписи"
  metrics:
    build_time_minutes: 15
    teammates_count: 2
    viewers_joined: 15
  prerequisites: ["Claude Code", "Opus 4.6"]
  related:
    - slug: "agent-consilium-independent-opinions"
      relation: "консилиум — про качество решений, Teams — про скорость выполнения"
    - slug: "blog-post-pipeline"
      relation: "пример использования субагентов в последовательном пайплайне"
    - slug: "ai-coding-tools-economics-2026"
      relation: "экономика токенов — Teams расходуют заметно больше"
---
