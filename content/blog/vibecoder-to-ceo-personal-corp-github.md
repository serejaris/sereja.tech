...
date: 2026-02-23
description: "Три уровня от промпта к Personal Corp: человек с терминалом, менеджер issues, CEO оркестратора. GitHub Issues — командный язык для роя агентов. Кейс issue #74."
...
На третьем — CEO. Ты описываешь задачу, оркестратор сам декомпозирует, раздаёт агентам, проверяет результат. Агенты работают параллельно — и не только над кодом: тексты, аналитика, стратегия. Опционально между тобой и Claude Code может стоять [OpenClaw](https://t.me/OpenClawBot) — высокоуровневый секретарь в Telegram, который передаёт задачи агенту на домашнем сервере. Про [настройку за $6/мес](/blog/openclaw-vps-6-dollars/) я уже писал.
...
Я запускаю [предзапись на Personal Corp](https://t.me/hashslash_bot?start=pc_blog) — формат "компания одного человека с агентами". Нужен был бот-флоу: deeplink, обработчик в Telegram-боте, сегмент для рассылок, terminal-стиль экран с текстом.
...
Возьми issue предзаписи Personal Corp. Разбей на подзадачи и реализуй: бот-флоу, экран, сегмент для рассылок. Коммиты помечай номером issue. Когда сделаешь — отчитайся комментарием и поставь статус Done.
{{< /callout >}}
...
1. Оркестратор разбил задачу на части: deeplink, хендлер, сегмент, terminal-стиль экран
2. Агенты параллельно создали код, тексты, кнопки-реакции с трекингом
3. Коммиты с пометкой (#74) — каждый привязан к issue
4. Оркестратор отчитался комментарием и поставил Done
...
Если хочешь попробовать — [забронируй место в Personal Corp](https://t.me/hashslash_bot?start=pc_blog). Расскажу, как это устроено изнутри.
