...
knowledge:
  problem: "Агент сразу бросается писать код без планирования, что приводит к переделкам"
  solution: "Использовать Superpowers скиллы с воркфлоу brainstorming → design → plan → execute"
  pattern: "brainstorming-before-code"
  tools: ["Claude Code", "Superpowers", "Exa MCP", "Payload CMS"]
  takeaways:
    - "SQLite на Railway теряет данные при каждом деплое из-за ephemeral filesystem"
    - "Полчаса brainstorming + plan, полчаса реализация — час вместо недели"
    - "Superpowers — 48K звёзд на GitHub, skill-first protocol заставляет агента думать перед кодом"
    - "LLM без веб-поиска предложит устаревшие решения — Exa MCP обязательная часть brainstorming"
    - "Plan — готовый промпт для агентов в чистой сессии, каждая задача изолированная"
  metrics:
    brainstorming_minutes: 30
    execution_minutes: 30
    superpowers_github_stars: 48000
  related:
    - slug: "skills-knowledge-transfer"
      relation: "Скиллы как способ переносить знания между проектами"
    - slug: "claude-code-token-optimization"
      relation: "Экономия контекста Claude Code"
    - slug: "agent-teams-opus-4-6"
      relation: "Команда агентов для параллельной реализации плана"
---
