...
Такие хуки — часть guardrails, которые заменяют мне approve-режим: контроль через контекст, а не через клики yes-yes-yes. Про этот подход я писал в [посте про bypass-режим без approve](/blog/cc-alias-bypass-permissions/).
