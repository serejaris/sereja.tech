---
title: "Jev и модели System One: разбор анонса TypeSafe"
date: 2026-09-17
description: "Разбор анонса TypeSafe: модель Jev не генерирует текст, а отдаёт коду типизированные решения с вероятностями. Откуда 193,6× и 444,6× и что настораживает."
...
Публичных бенчмарков нет, и их не будет. У TypeSafe целая статья о том, почему публичные таблицы ломают стимулы: под них оптимизируются, приватные варианты тестируют пачками и публикуют лучший. Взамен они обещают разовые оценки к каждому обновлению продукта и призывают клиентов строить собственные evaluation. Логика честная, но следствие простое: проверять придётся на своих задачах, доверять таблицам не получится.

«Не галлюцинирует» звучит сильнее, чем означает. Гарантия покрывает схему и типы: модель не может выдать значение не того типа или поле вне схемы. Идеально оформленная бессмыслица с правильными типами остаётся возможной, поэтому калиброванные вероятности рядом с каждым ответом важнее самого запрета галлюцинаций.

Цена вызывает вопросы у них же самих. Ноль целых ноль сорок два сотых доллара за миллион входных токенов и бесплатные выходные, с прямой оговоркой: доказать, что это не субсидия, они пока не могут.

И главное: модель в раннем доступе. Список ожидания, ограниченные воркфлоу, мало независимых проверок. Всё выше категория «посмотрим своими руками».
