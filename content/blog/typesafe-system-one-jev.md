...
## Попробовать в Telegram без установки

В нашем [@jev_robot](https://t.me/jev_robot) можно посмотреть, как Jev решает небольшой вопрос, без своего API-ключа. Открой бота, нажми Start → «Разобрать пример». Карточка сначала показывает вход, критерии, источник и схему; запрос к модели запускает кнопка «Проверить пример · 1 запрос».

Возьми карточку «Какую кнопку выбрать?». Цель задаёт команда «открой документацию». В описании страницы три элемента: `e1`: ссылка «Документация», `e2`: кнопка «Войти», `e3`: поле «Поиск». Бот передаёт Jev этот текст и вопрос Choice: какой элемент соответствует команде, либо ни один? Команде соответствует `e1`, ссылка «Документация». [Бот](https://t.me/jev_robot) показывает выбранный моделью вариант, вероятности и уверенность; «Ответ API» открывает JSON. Сопоставь выбранный идентификатор с элементом из входа. Бот показывает выбор, браузерную кнопку он не нажимает.

Сверь ответ с командой и доступными элементами: выбор «Войти» эту цель не выполняет. Затем через «Проверить мою задачу» замени вход при тех же критериях. Результат одного примера не гарантирует правильность следующего; высокая уверенность тоже не заменяет проверки.

В [«Все примеры»](https://t.me/jev_robot) пять иллюстрированных карточек: выбор кнопки, проверка команды агента, режим модели, поиск функции по смыслу и риск изменения кода. Это учебные текстовые адаптации открытых проектов. Бот оценивает описания, команды и код не исполняет, модель сам не переключает. «Свой вопрос» позволяет задать собственный Choice, Score или Noul с нужными вариантами либо уровнями.

На 17 сентября проба в [боте](https://t.me/jev_robot) бесплатная в пределах 20 запросов в сутки по UTC. Просмотр карточек и сохранённого ответа квоту не расходует; один запрос к модели списывает одну единицу. Между запросами нужны 10 секунд, текст ограничен 3000 символами. Доступ зависит от бюджета сервиса и доступности API. Введённый текст и критерии передаются через Vercel AI Gateway в TypeSafe; не отправляй секреты и личные данные.
