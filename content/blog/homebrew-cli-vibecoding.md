...
faq:
  - q: "Нужно ли знать терминал, чтобы использовать Homebrew с Claude Code?"
    a: "Нет. Агент сам устанавливает и запускает программы. Достаточно описать задачу словами — «скачай видео», «сконвертируй в mp3». Homebrew нужно установить один раз, дальше агент работает самостоятельно."
  - q: "Чем Homebrew лучше ручной установки?"
    a: "Homebrew разрешает зависимости, обновляет программы, проверяет совместимость. Агент ставит ffmpeg одной строкой. Без Homebrew — скачивание бинарников, настройка путей, конфликты версий."
  - q: "Работает ли это на Linux?"
    a: "Да, Homebrew поддерживает Linux. ffmpeg, yt-dlp, jq работают одинаково на обеих платформах."
---
...
### Перестать печатать пути

**zoxide + fzf** — умный `cd` из того же каталога brew. Запоминает директории, куда ты ходишь, и прыгает по ключевому слову: `z blog` вместо `cd ~/Documents/GitHub/sereja.tech`. Полезно и тебе в терминале, и агенту, который ходит между десятком репозиториев в сессии. [Настройку и грабли разобрал в отдельном посте](/blog/zoxide-fzf-terminal-navigation/).
