...
knowledge:
  problem: "Создание карточек для Telegram-каруселей в Figma/Canva занимает час и неудобно при частых правках"
  solution: "HTML/CSS шаблон с автоматическими скриншотами через Playwright — текст в коде, стиль в CSS"
  pattern: "html-to-image-pipeline"
  tools: ["Claude Code", "Playwright", "HTML/CSS", "Google Fonts", "Python"]
  takeaways:
    - "6 карточек 1080×1080 сгенерированы с первого промпта"
    - "HTML-карточки лежат в git — откат к прошлой версии одной командой"
    - "CSS-переменные обеспечивают консистентность: отступы, шрифты, цвета"
    - "Playwright скриншотит отдельные элементы по id"
---
