...
knowledge:
  problem: "AI-агент теряет контекст между сессиями и не логирует выполненные команды автоматически."
  solution: "Три хука OpenClaw — session-memory, boot-md и command-logger — добавляют персистентность без участия LLM и без токенов."
  pattern: "event-driven-hooks"
  tools: ["OpenClaw", "Telegram", "Kimi K2.5", "TypeScript"]
  takeaways:
    - "Хуки выполняются без LLM — ноль токенов, мгновенное выполнение"
    - "Три промпта и ноль написанного вручную кода кардинально изменили поведение агента"
    - "Проектные хуки перекрывают глобальные — можно переопределить поведение для конкретной задачи"
    - "BOOT.md при старте — аналог CLAUDE.md для Claude Code, только для OpenClaw-агента"
  prerequisites: ["VPS с установленным OpenClaw"]
  related:
    - slug: "openclaw-vps-6-dollars"
      relation: "Первая часть — установка OpenClaw на VPS"
    - slug: "pipeline-reliability-state-persistence"
      relation: "Тот же принцип сохранения состояния, но для пайплайнов"
    - slug: "personal-corporation-event-driven-agents"
      relation: "Хуки — конкретная реализация event-driven подхода внутри одного агента"
---
