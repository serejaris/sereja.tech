...
knowledge:
  problem: "Посты в Telegram-канал выходят хаотично — то три за день, то неделю тишина"
  solution: "Автопостер с очередью: добавляешь текст, система назначает дату, launchd отправляет в 19:00 МСК"
  pattern: "queue-based-autoposter"
  tools: ["Claude Code", "Telegram Bot API", "launchd", "Python"]
  takeaways:
    - "Постоянное расписание даёт в 5 раз больше вовлечённости (исследование Buffer)"
    - "Система из 4 частей: queue.json, queue.py, scheduler.py, launchd"
    - "SaaS-альтернативы стоят $10-50/месяц, своё решение бесплатно"
    - "Логика дат: пустая очередь — сегодня/завтра, есть посты — следующий свободный день"
  metrics:
    engagement_multiplier: 5
    posting_time: "19:00 MSK"
    saas_cost_range: "$10-50/month"
---
